...
class Entry(TypedDict, total=False):
    """Parsed feed-item dict produced by RSS/Atom parsers and consumed everywhere else."""
...
def html_to_text(html: str, width: int = 80) -> str:
...
def fetch_feed(url: str, cookies: str | None = None) -> ET.ElementTree:
...
def get_atom_entries(root: ET.Element) -> list[Entry]:
...
def get_entries(tree: ET.ElementTree) -> list[Entry]:
...
def show_article(entry: Entry, width: int = 80) -> None:
...
def fetch_single_feed(name: str, feed_value: FeedConfig) -> tuple[str, list[Entry], Exception | None]:
    ...
    try:
        ...
        for entry in entries:
            entry["source"] = name
    except Exception as e:  # noqa: BLE001  # third-party feed; survive any failure mode
        return (name, [], e)
    return (name, entries, None)


def fetch_all_feeds(feeds: dict[str, FeedConfig]) -> list[Entry]:
...
def main():
    ...
    if args.feed is None:
        ...
        except Exception as e:  # noqa: BLE001  # third-party feed; surface any failure to user
